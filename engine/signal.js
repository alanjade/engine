...
export function evaluate({ symbol, candles4h, candles1d, position, openPositionCount }) {
  ...
  // ── 1. Data validity ───────────────────────────────────────────────────────
  if (
    !candles4h?.length || !candles1d?.length ||
    candles4h.length < 100 || candles1d.length < 100
  ...
  if (!d_ema50 || !d_ema200)
    return none(symbol, 'Insufficient 1D data for EMA.');
  const dailyBull = d_ema50 > d_ema200;
  if (!dailyBull)
    return none(symbol, 'Daily trend bearish — no BUY.');

  // ── 5. 4H EMA trend strength ───────────────────────────────────────────────
  const ema50  = calcEMA(closes4h, 50);
  const ema200 = calcEMA(closes4h, 200);
  if (!ema50 || !ema200)
    return none(symbol, 'Insufficient 4H data for EMA.');

  const emaDistPct = ((ema50 - ema200) / ema200) * 100;
  if (ema50 < ema200)
    return none(symbol, '4H EMA50 below EMA200.');
  if (price < ema50)
    return none(symbol, 'Price below 4H EMA50.');
  if (emaDistPct <= c.emaDistMin)
    return none(symbol, `EMA distance ${round2(emaDistPct)}% ≤ ${c.emaDistMin}% threshold.`);

  // ── 6. Sideways filter ─────────────────────────────────────────────────────
  const crosses = emaCrossCount(candles4h, 50, 200, 20);
  if (crosses >= 2)
    return none(symbol, 'Multiple EMA crosses in last 20 candles — sideways market.');

  // ── 7. Support quality ─────────────────────────────────────────────────────
  const support = nearestSupport(candles4h, price, c.supportLookback);
  if (!support)
    return none(symbol, `No valid support found (lookback ${c.supportLookback}).`);
  const supScore = supportScore(support);
  if (supScore < 2)
    return none(symbol, `Support score ${supScore} < 2.`);

  // ── 8. Resistance quality ──────────────────────────────────────────────────
  const resistance = nearestResistance(candles4h, price, c.supportLookback);
  if (!resistance)
    return none(symbol, `No valid resistance found (lookback ${c.supportLookback}).`);
  const resScore = resistanceScore(resistance);
  if (resScore < 2)
    return none(symbol, `Resistance score ${resScore} < 2.`);
  ...
  if (volumeRatio < c.volRatioMin)
    return none(symbol, `Volume ratio ${round2(volumeRatio)} < ${c.volRatioMin}.`);

  // ── 10. RSI confirmation ───────────────────────────────────────────────────
  const rsi = calcRSI(closes4h, 14);
  if (rsi === null)
    return none(symbol, 'RSI calculation failed.');
  if (rsi < c.rsiMin || rsi > c.rsiMax)
    return none(symbol, `RSI ${round2(rsi)} outside ${c.rsiMin}–${c.rsiMax} buy zone.`);

  // ── 11. ATR filter ─────────────────────────────────────────────────────────
  const atr    = calcATR(candles4h, 14);
  const atrPct = atr ? (atr / price) * 100 : 0;
  if (!atr || atrPct < c.atrMin)
    return none(symbol, `ATR% ${round2(atrPct)} < ${c.atrMin}% minimum.`);

  // ── 12a. Price proximity to support ───────────────────────────────────────
  ...
  if (rr < c.minRR)
    return none(symbol, `RR ${round2(rr)} < ${c.minRR} minimum.`);
...
function evaluateSell({ symbol, candles4h, position, price, now }) {
  ...
  if (
    position.resistance &&
    price >= position.resistance * 0.99 &&
    price <= position.resistance * 1.01
